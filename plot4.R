...
## Turn on PNG graphic device to successfully save file in legible format in local working dir
   png(file="plot4.png",width=480,height=480)

## Make 2x2 mulitple plots (via graphic parms) and 
   par(mfcol = c(2,2))
   with(data4, {
  ## Plot 1
  ...
  ## Plot 2
     ...
     legend("topright", col= c("black","red","blue"), legend = colnames(data4[6:8]), lty=c(1,1,1))
  
  ## Plot 3 (and control the y axis tick marks)
     plot(Date,Voltage, type = "l", ylab = "Voltage", xlab  = "datetime",  yaxt="n")
     c = seq(from = ceiling(min(data4$Voltage)),to = ceiling(max(data4$Voltage)), by = 4)
     axis(2, at=c, labels = c)
